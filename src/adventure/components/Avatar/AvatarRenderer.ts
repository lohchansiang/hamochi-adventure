...
import { AvatarTextureRepo } from "@/adventure/repos/AvatarTextureRepo";
import AvatarMaskRenderer from "./AvatarMaskRenderer";
import { AvatarInterface } from "./AvatarInterface";

export default class AvatarRenderer implements AvatarInterface{
    ...
    renderAvatar( characterKey:string ){
        ...
        if( this.charData && this.charData.avatar ){
            let bodyBaseTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.bodyBase);
            let bodyStrokeTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.bodyStroke);
            let mouthTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.mouth);
            let eyeTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.eye);
            ...
            if( this.charData.avatar.hair.length > 0 ){
                let hairTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.hair[0]);
            ...
            let bodyShadowTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.shadow);
    ...
    private prepareAnims(){
        ...
        if( this.charData ){
            let eyeTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.eye);
            let eyeBlinkTexture: string = AvatarTextureRepo.getTextureKey(this.charData.avatar.eyeBlink);
        ...
        if( this.charData 
            && this.charData.avatar 
            && this.charData.avatar.hair
            && this.charData.avatar.hair.length > 1
        ){
            // Prepare Frames
            let frames: { key: string; }[] = [];
            this.charData.avatar.hair.forEach((textureKey)=>{
                frames.push({key: AvatarTextureRepo.getTextureKey(textureKey) });
            });
